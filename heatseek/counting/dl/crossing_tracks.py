...
# def visualize_crossing_tracks(crossing_tracks_file, midline_y=88, num_tracks=100):
...
#     print(f'Forward crossings: {sum(1 for t in crossing_tracks if t["crossed"] > 0)}')
#     print(f'Backward crossings: {sum(1 for t in crossing_tracks if t["crossed"] < 0)}')
...
#         color = 'blue' if track['crossed'] > 0 else 'red'
...
#     ax = axes[1]
#     crossing_frames = [abs(t['crossed']) for t in crossing_tracks]
...
#         direction = 'forward' if t['crossed'] > 0 else 'backward'
#         print(f'  frame {abs(t["crossed"])}: {direction}')
